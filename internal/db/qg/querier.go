// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.29.0

package qg

import (
	"context"

	"github.com/jackc/pgx/v5/pgtype"
)

type Querier interface {
	AddVideoToPlayer(ctx context.Context, arg AddVideoToPlayerParams) (Video, error)
	AdvanceGames(ctx context.Context, batchSize int32, enqueueInterval pgtype.Interval) ([]AdvanceGamesRow, error)
	CountPlayerInGame(ctx context.Context, gameID int64) (int64, error)
	CreatePlayer(ctx context.Context, arg CreatePlayerParams) (Player, error)
	CreateRound(ctx context.Context, arg CreateRoundParams) (Round, error)
	CreateTheme(ctx context.Context, title string, description pgtype.Text) (Theme, error)
	FetchVotesByPlayers(ctx context.Context, gameID int64, roundN int32) ([]FetchVotesByPlayersRow, error)
	GetGameAndLock(ctx context.Context, id int64) (GetGameAndLockRow, error)
	GetGameByPlayer(ctx context.Context, gameID int64, playerID int64) (GetGameByPlayerRow, error)
	GetGameByPlayerAndStage(ctx context.Context, arg GetGameByPlayerAndStageParams) (Game, error)
	GetGameRounds(ctx context.Context, id int64) ([]Round, error)
	GetGameVideosForVote(ctx context.Context, arg GetGameVideosForVoteParams) (Game, error)
	GetPlayerByTgId(ctx context.Context, tgID int64) (Player, error)
	GetSubmittedVideosByPlayers(ctx context.Context, gameID int64, roundN int32) ([]GetSubmittedVideosByPlayersRow, error)
	GetTheme(ctx context.Context, id int64) (Theme, error)
	GetVideosByPlayer(ctx context.Context, playerID int64) ([]Video, error)
	GetVideosToWatch(ctx context.Context, gameID int64, roundN int32) ([]GetVideosToWatchRow, error)
	JoinGame(ctx context.Context, arg JoinGameParams) (int64, error)
	ListAllThemes(ctx context.Context) ([]Theme, error)
	ListThemes(ctx context.Context, limit int32, offset int32) ([]Theme, error)
	SetCompletedStatus(ctx context.Context, stage GameStage, gameID int64) (GameStatus, error)
	TestAddPlayerToGame(ctx context.Context, gameID int64, playerID int64) (GamePlayer, error)
	TestCreateGame(ctx context.Context, themeID int64) (Game, error)
	TestGetGameById(ctx context.Context, id int64) (Game, error)
	UpdateGameStatus(ctx context.Context, arg UpdateGameStatusParams) (UpdateGameStatusRow, error)
	UpdateRemainingTime(ctx context.Context, nextStateIn pgtype.Interval, gameID int64) (GameStatus, error)
	UpsertGameVideoIfOwned(ctx context.Context, arg UpsertGameVideoIfOwnedParams) (GameVideo, error)
	VoteForVideo(ctx context.Context, arg VoteForVideoParams) (GameVote, error)
}

var _ Querier = (*Queries)(nil)
